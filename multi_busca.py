# -*- coding: utf-8 -*-
"""
Created on Wed Jun 14 14:52:52 2023
...
dataframe = pd.read_excel(R'C:\Users\gabriel.rodrigues\Desktop\lista_apos_busca_park.xlsx')                   
diretorio = R'C:\Users\gabriel.rodrigues\Desktop\teste 2\exclsuive'
empreendimentos = os.listdir(diretorio)
conjunto_arquivos = []
...
def processar_lista(conjunto):
    
    elemento_buscado = verificar_correspondencia(conjunto)
    print('aqui')
    if elemento_buscado:
...
for bloco in empreendimentos:
    ...
    for apartamento in blocos:
        ...
        for arquivo in apartamentos:
            ...
            if teste3.lower() == 'pdf':
                conjunto_arquivos.append(diretorio5)
                ...
                for documento in arquivos:
                    ...
                    if teste30.lower() == 'pdf':
                        conjunto_arquivos.append(diretorio6) 
        
print('possui {} documentos pdf para analisar'.format(len(conjunto_arquivos)))
num_processos = 4
pool = multiprocessing.Pool(processes=num_processos)   
try:  
    resultados = pool.map(processar_lista, conjunto_arquivos)

    pool.close()

# Aguarda até que todos os processos sejam concluídos
    pool.join()
except KeyboardInterrupt:
    pool.terminate()

        # Espera que os processos terminem completamente
    pool.join()
print(resultados)


dataframe = pd.DataFrame(resultados, columns=['Nome', 'cota','quantidade de paginas', 'situacao']) 
