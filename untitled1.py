...
def extrair_nome_empre(caminho_pdf):
    with open(caminho_pdf, 'rb') as pdf_file:
        ...
        # Expressão regular para encontrar padrões de nome do cliente
        padrao_nome_empre = r"DESCREVE;\s*(\w+\s+\w+)"
...
def extrair_nome_cota(caminho_pdf):
    with open(caminho_pdf, 'rb') as pdf_file:
        ...
        # Expressão regular para encontrar padrões de nome do cliente
        padrao_nome_cota = r"2Quartos\s*(\w+)"
...
def extrair_nome_bloco(caminho_pdf):
    with open(caminho_pdf, 'rb') as pdf_file:
        pdf_reader = PyPDF2.PdfReader(pdf_file)
        primeira_pagina = pdf_reader.pages[1]
        texto = primeira_pagina.extract_text()
        ...
        # Expressão regular para encontrar padrões de nome do cliente
        padrao_nome_bloco = r"Cotanº.:\s*(\w+\s+\w+\s+\w+)"

        # Procurar o padrão no texto
        match = re.search(padrao_nome_bloco, texto)
        ...
        if match:
            ...
            return None
diretorio = R'W:\ARQUIVOS-GNA\PUBLICA\Pub. Coordenação SALA\CONTRAKTOR\Vigente_\arquivozip\0003'
...
for pasta in pastas:
    ...
    nome_bloco = extrair_nome_bloco(caminho_arquivo_pdf)
    
    caminho_antigo = os.path.join(diretorio, pasta)
    
    if nome_cliente and nome_empre and nome_cota and nome_bloco:
        i2=str(i)
        novo_nome_pasta = i2 +'-'+ nome_cliente +'-'+  nome_empre +'-'+  nome_bloco + '-' + nome_cota
