...
lotes = [conjunto_arquivos[i:i+tamanho_lote] for i in range(0, len(conjunto_arquivos), tamanho_lote)]


for elemento in conjunto_arquivos:
    print('verificando o {}º arquivo '.format(contador))
    elemento_buscado = pool.map(verificar_correspondencia, elemento)
    print('verificando o {}º arquivo '.format(contador))
    contador = contador + 1
    for lote in elemento:
        if elemento_buscado !='':
